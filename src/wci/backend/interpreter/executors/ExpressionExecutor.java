...
public class ExpressionExecutor extends StatementExecutor 
{
	...
	public Object execute(ICodeNode node)
	{
		...
		switch (nodeType)
		{
			...
			case INTEGER_CONSTANT: {
				...
				// Returns a boolean if type is boolean, otherwise returns the integer value
			...
			case CALL: {
				...
				// If it was a declared function, obtain its value from its name
	...
	public Cell executeVariable(ICodeNode node) {
		...
		// If variable is a reference to a reference, use original reference
		if (variableCell.getValue() instanceof Cell) {
			variableCell = (Cell) variableCell.getValue();
		}

		// Execute any array references or record fields
		for (ICodeNode modifier : modifiers) {
			...
			if (nodeType == SUBSCRIPTS) {
				...
				// Compute a new reference to the field
	...
	private Object executeBinaryOperator(ICodeNode node, ICodeNodeTypeImpl nodeType) {
		...
		if (operand1 instanceof Integer && operand2 instanceof Integer) {
			integerMode = true;
		} else if (isCharOrStringOfSizeOne(operand1) && isCharOrStringOfSizeOne(operand2)) {
			characterMode = true;
	...
	private boolean isCharOrStringOfSizeOne(Object operand) {
		return operand instanceof Character || (operand instanceof String && ((String) operand).length() == 1);
	}
